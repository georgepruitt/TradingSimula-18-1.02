...
sysName = 'TF-TMA' #System Name here
initCapital = 500000
commission = 50
...
#            upBand,dnBand, midBand = bollingerBands(myDate,myClose,80,2,curBar,1)
            sma = sAverage(myClose,39,curBar,1)
            ima = sAverage(myClose,89,curBar,1)
            lma = sAverage(myClose,299,curBar,1)
...
#  Okay  Let's put in some logic to create a short position
            if sma < ima and sma < lma and ima < lma and mp !=-1:
                ...
                if mp >= 1:
                    ...
                    marketMonitorList[curMarket].tradesList.append(trades);todaysCTE = profit
                tradeName = "TF-TMA-S";numShares = posSize
