...
#    lineOutPut =
    target5.write
    fileName6 = systemMarketList[0].systemName + "-EquityMatrix.txt"
    target6 = open(fileName6,"w")

    tradeTuple = list()
    masterEquityMatrix = list()
    ...
    for i in range(0,len(systemMarketList)):
        ...
        target1.write("\n")
        if printToTerminal == 1: print("Mkt Symb : ",systemMarketList[i].symbol," Mkt Name : ",systemMarketList[i].comName)
        lineOutPut = "Mkt Symb : " + systemMarketList[i].symbol + " Mkt Name : " + systemMarketList[i].comName
        target1.write(lineOutPut)
        target1.write("\n")
        ...
        if printToTerminal == 1: print("Avg DrawDown: ",round((systemMarketList[i].avgMaxDD),2))
        lineOutPut = "Avg DrawDown: " + str(round((systemMarketList[i].avgMaxDD),2))
        target1.write(lineOutPut)
        target1.write("\n")
...
#            systemMarketList[i].tradesList[j].printTrade()
            if j < len(systemMarketList[i].tradesList)-1:
                if systemMarketList[i].tradesList[j].tradePrice != systemMarketList[i].tradesList[j+1].tradePrice:
                    row = str(systemMarketList[i].tradesList[j].tradeDate) +","+systemMarketList[i].symbol+","+str(systemMarketList[i].tradesList[j].quant)+","
...
#            print(systemMarketList[i].equity.equityDate[j],' ',systemMarketList[i].equity.dailyEquityVal[j])
        ...
        for j in range(0,numOfMonths):
            ...
            if isDecember == False:
                ...
                masterEquityMatrix +=((systemMarketList[i].symbol,j,cumulativeEquity[j]),)
            if isDecember:
    ...
    target2.close()
    target5.close()
# out put MasterEquityMatrix
    lineOutPut = ""
    for x in range(0,len(systemMarketList)):
        lineOutPut += systemMarketList[x].symbol + ','
    target6.write(lineOutPut)
    target6.write('\n')
    for x in range(0,numOfMonths):
        lineOutPut = ""
        lineOutPut = str(monthList[x]) + ","
        for y in range(0,len(masterEquityMatrix)):
            if masterEquityMatrix[y][1] == x:
                lineOutPut += str(round(masterEquityMatrix[y][2],2)) + ","
        target6.write(lineOutPut)
        target6.write('\n')

# start trade draw down analysis - utilizing the tradeTuple
    tupleLen = len(tradeTuple)
#    print("tradeTuple", len(tradeTuple))
    if tupleLen > 1:
 ...
 #               if x == 0: print("Trade Tuple ",tradeTuple[y][0]," ",tradeTuple[y][1]);
